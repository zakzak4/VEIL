...
let markers = [];
let prepared = false;
const elementDistanceToPoint = '.point-distance';
const elementInputPostcode = '#input-shipping-postcode';
const elementInputAddress = '#input-shipping-address-1';
const elementInputCity = '#input-shipping-city';
const elementInputPostcodeSecondary = '#input-payment-postcode';
const elementInputAddressSecondary = '#input-payment-address-1';
const elementInputCitySecondary = '#input-payment-city';
const pluginFolder = '/image/catalog/acs-points-api/';
...
jQuery(document).on('click', '.selected', function (e) {
    ...
    if (window['_QuickCheckoutData'] !== undefined) {
        window['_QuickCheckoutData']['order_data']['acs_pp_point_id'] = JSON.stringify(locations[key].id);
    }
...
function postcodeSearch(postcode, street, alert_error = true, country = 'GR') {
    if (!prepared) {
        prepare();
    }
    ...
    address = address + "ΤΚ " + postcode + ", GR";
...
function openMap() {
    ...
    postcodeSearch(
        jQuery(elementInputPostcode).val() ?? jQuery(elementInputPostcodeSecondary).val(),
        (jQuery(elementInputAddress).val() ?? jQuery(elementInputAddressSecondary).val()) + ',' + (jQuery(elementInputCity).val() ?? jQuery(elementInputCitySecondary).val())
    );
    toggleMapModal();
}
